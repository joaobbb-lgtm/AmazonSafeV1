...
# - Chamadas diretas a uvicorn.run(...) e blocos if __name__ == "__main__": foram comentados.
...
# ==== [cell] =============================================
## 7) FastAPI — rotas (consolidado)

import nest_asyncio, uvicorn, json, time, os, io, csv
...
# ==== [cell] =============================================
# ## 9) Endpoints de Dados — consolidados e persistência (com seleção de provedor de clima)
...
# -------------------------
# GET /api/data (consolidado por cidade)
# -------------------------
@app.get(
    "/api/data",
...
# ==== [cell] =============================================
## 8) Subir o servidor (Uvicorn + Cloudflared) — consolidado
# @title Subir servidor + túnel (cloudflared)
import os, time, threading, subprocess, re, nest_asyncio, uvicorn, requests
...
def _have_cloudflared() -> bool:
    return bool(
        which("cloudflared")
        or os.path.exists("/usr/bin/cloudflared")
        or os.path.exists("/usr/local/bin/cloudflared")
    )

def _wait_http_ok(url: str, tries: int = 30, interval: float = 1.0) -> bool:
...
def print_endpoints(base_url: str):
    base = base_url.rstrip("/")
    lat, lon = "-1.4558", "-48.5039"
    cidade_q = quote("Belém, PA")

    print(f"✅ Public URL: {base}")
    print(f"Swagger UI: {base}/docs")
    print(f"Health: {base}/health")
    print(f"Alertas (HTML): {base}/alertas.html")
    print(f"Alertas (JSON): {base}/api/alertas?limit=50")
    print(f"Alertas (latest por cidade): {base}/api/alertas/latest")
    print(f"Alertas (CSV): {base}/api/alertas.csv")
    print(f"Weather: {base}/api/weather?lat={lat}&lon={lon}")
    print(f"Air (OpenAQ v3): {base}/api/air/openaq?lat={lat}&lon={lon}&radius_m=25000")
    print(f"INPE (focos CSV): {base}/api/inpe/focos?lat={lat}&lon={lon}&raio_km=150&scope=diario&region=Brasil&limit=200")
    print(f"ANA (stub): {base}/api/ana/estacoes?cidade={quote('Manaus, AM')}&raio_km=150&limit=3")
...
def _cron_loop():
    iv = MINI_CRON["interval_sec"]
    url = MINI_CRON["base_url"].rstrip("/") + "/api/alertas_update"
    tout = MINI_CRON["timeout_sec"]
    ev = MINI_CRON["stop_event"]
    body = MINI_CRON["body"]

    print(f"[mini-cron] iniciado: {iv}s -> POST {url} body={body}")
    while not ev.is_set():
        t0 = time.time()
        try:
            r = requests.post(url, json=body, timeout=tout)
            ok = (200 <= r.status_code < 300)
            print(f"[mini-cron] {datetime.now().isoformat(timespec='seconds')} -> status={r.status_code}, ok={ok}")
        except Exception as e:
            print(f"[mini-cron] {datetime.now().isoformat(timespec='seconds')} -> ERRO: {e}")

        dt = time.time() - t0
        wait_left = max(0.0, iv - dt)
        if ev.wait(wait_left):
            break
    print("[mini-cron] parado.")

def start_mini_cron(every_minutes: int = 5, base_url: str | None = None, body_override: dict | None = None):
    if MINI_CRON["thread"] and MINI_CRON["thread"].is_alive():
        print("[mini-cron] já está em execução.")
        return

    MINI_CRON["interval_sec"] = max(30, int(every_minutes) * 60)  # mínimo 30s
    if base_url:
        MINI_CRON["base_url"] = base_url
    if body_override:
        MINI_CRON["body"] = {**MINI_CRON["body"], **body_override}
...
def start_mini_cron_when_ready(
    ...
    for i in range(1, tries + 1):
        try:
            ...
            if 200 <= r.status_code < 300:
                ready = True
                print(f"[mini-cron] servidor OK (tentativa {i}/{tries})")
                break
        except Exception:
            pass
        time.sleep(interval_check_sec)
...
def stop_mini_cron():
    ev = MINI_CRON.get("stop_event")
    thr = MINI_CRON.get("thread")
    if ev and thr and thr.is_alive():
        ev.set()
        thr.join(timeout=5)
        print("[mini-cron] join concluído.")
    else:
...
def cron_print_status():
    print(f"running: {cron_is_running()}")
    print(f"interval_sec: {MINI_CRON.get('interval_sec')}")
